...
import xlrd #excel para usar com pandas 2003
import xlwt
import os
...
#with pd.ExcelWriter(caminho_planilha, engine=None, mode='a', if_sheet_exists='replace') as writer:


    # O método 'to_excel' do DataFrame 'resultados' é chamado para escrever seus
                # dados na aba chamada 'Dados' do arquivo Excel.xlrd
...
for index, row in resultados.iterrows():
    linha = linha+1 
    sheet.write(linha, 0, row['CEP']) 
    print(f"CEP: {row['CEP']}")
    sheet.write(linha, 1, row['Logradouro']) 
    print(f"Logradouro: {row['Logradouro']}")
    sheet.write(linha, 2, row['Bairro']) 
    print(f"Bairro: {row['Bairro']}")
    sheet.write(linha, 3, row['Localidade']) 
    print(f"Localidade: {row['Localidade']}") #'CEP', 'Logradouro', 'Bairro', 'Localidade', 'UF'
    sheet.write(linha, 4, row['UF']) 
    print(f"UF: {'UF'}")
    print("-" * 20)
planilhaCriada.save(caminho_planilha )
# Imprime uma mensagem indicando que os endereços foram salvos na
                # aba 'Dados' do arquivo especificado.
os.startfile("CEP.xls")                
