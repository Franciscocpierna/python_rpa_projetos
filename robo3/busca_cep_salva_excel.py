...
def salvar_endereco_excel(endereco, nome_arquivo="endereco.xls"):
    """
    Salva os dados de endereço obtidos da API em um arquivo Excel 2003 (.xls), 
    ...
    nome_arquivo (str): Nome do arquivo Excel para salvar os dados. 
                Se não especificado, 'endereco.xls' será usado.
    """
    ...
    if "erro" not in endereco:
        ...
        sheet = workbook.add_sheet("Endereço")
        
        # Escreve os cabeçalhos (nomes das colunas)
        for col, key in enumerate(endereco.keys()):
            sheet.write(0, col, key)  # Linha 0, Coluna `col`
        
        # Escreve os valores do endereço
        for col, value in enumerate(endereco.values()):
            sheet.write(1, col, value)  # Linha 1, Coluna `col`
