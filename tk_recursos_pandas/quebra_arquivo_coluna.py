...
class ExcelEditor:
    ...
    def __init__(self, janela_principal):
        ...
        #Programa o evento de duplo clique para passar os dados
        #Da treeview para a tela de edição
    ...
    def cria_widgets(self):
        ...
        relatorio_menu.add_command(label="Quebra", command=self.quebra_arquivo)
    ...
    def merge_join_full(self):
        ...
        #Realiza o merge utilizando o tipo "full", ou seja, une todos os registros de ambos os arquivos
        self.df = pd.concat([arquivo1, arquivo2])
        
        #Remove as linhas em duplicidades
    ...
    def merge_left_join(self):
        ...
        #Pergunta ao usuário qual coluna deve ser utilizada no merge
        #coluna = simpledialog.askstring("Coluna Left Join", "Digite o nome da coluna que será utilizada para o Left Join:")
        ...
        #on - Coluna
        #Realiza o merge utilizando o tipo "left", ou seja, une todos os registros a partir da esquerda
        #self.df = pd.merge(arquivo1, arquivo2, on=coluna, how="left")
        self.df = pd.merge(arquivo1, arquivo2, on=["Id Vendedor"], how="left", suffixes=(" Vendas", " Checagem"))
        
        #Remove as linhas que tem pelo menos um valor em branco
    ...
    def merge_outer(self):
        ...
        #inner - Faz o merge entre as tabelas
        #Procura e exibe os vendedores ques estão em ambas as tabelas
        #self.df = pd.merge(arquivo1, arquivo2, on=coluna, how="outer")
        #suffixes - Muda o nome da coluna
    ...
    def salvar_como(self):
        ...
        nome_arquivo = filedialog.asksaveasfilename(title="Salvar como", filetypes=tipo_de_arquivo, defaultextension=".xlsx")
 
        if nome_arquivo:
        
            #Salva o arquivo em formato excel
            self.df.to_excel(nome_arquivo, index=False)
            
    def editarItens(self, event):
        
        #Obtem o item da treeview que foi clicado
        item = self.tree.selection()[0]
        
        #Obtem os valores do item selecionado
        values = self.tree.item(item, "values")
        
        #Cria a jabela de edição que a janela secundária
        janela_edicao = tk.Toplevel()
        
        #for - para
        # Adiciona campos de edição para cada coluna da tabela
        for linha, nome_coluna in enumerate(self.df.columns):
            
            # Cria um widget Label com o texto da coluna e o adiciona na janela de edição.
            label = tk.Label(janela_edicao, text=nome_coluna, font="Arial 20")
            label.grid(row=linha, column=0)
            
            # Cria um widget Entry vazio e o adiciona na janela de edição.
            entry = tk.Entry(janela_edicao, font="Arial 20")
            
            # Insere o valor atual da célula da coluna na Entry criada acima.
            # O valor atual é obtido a partir da lista 'values', que foi passada como parâmetro para a função.
            entry.insert(0, values[linha])
                        
            # Adiciona a Entry na janela de edição.
            entry.grid(row=linha, column=1)
        
        #Adiciona um botão de salvamento
        #O uso do lambda permite que a função salvar_alteracoes() seja chamada com os parâmetros 
        #corretos quando o botão for pressionado, sem a necessidade de definir explicitamente uma função nomeada para esse fim.
        salvar = tk.Button(janela_edicao, text="Salvar", 
                           command=lambda: self.salvar_alteracoes(item, janela_edicao))
        salvar.grid(row=len(self.df.columns), column=1)
        
    def salvar_alteracoes(self, item, janela_edicao):
          
        #Obtém os novos valores inseridos na janela de edição
        #A cláusula if isinstance(child, tk.Entry) é usada para filtrar apenas os widgets 
        #do tipo Entry na lista. Isso é feito usando a função isinstance() do Python, que verifica 
        #se o widget é uma instância da classe Entry do Tkinter.
        novos_valores = [child.get() for child in janela_edicao.winfo_children() if isinstance(child, tk.Entry)]
            
        #Atualiza o dataframe com os novos valores
        #O método loc do DataFrame é usado para selecionar a linha com o índice especificado
        #em item e todas as colunas representadas por : (ou seja, todas as colunas).
        self.df.loc[item, :] = novos_valores
            
        # Atualiza a treeview com os novos valores
    ...
    def consolidar_arquivos(self):
        ...
        try:
            ...
            #Pefamos o camino + nome e verificamos os 4 últimos digitos de cada arquivo
            #Se for xlsx seleciona ou coloca o arquivo na lista
            lista_caminho_e_arquivos_excel = [caminho_arquivos + '\\' + arquivo for arquivo in lista_arquivos if arquivo[-4:] == 'xlsx']
            
            #Cria o data frame
            dados_arquivo = pd.DataFrame()
            
            #Copia todos os dados dos arquivos para a variavel dados_arquivo
            for arquivo in lista_caminho_e_arquivos_excel:
                
                #lê os dados do arquivo que estiver passando na hora
                dados = pd.read_excel(arquivo)
                dados_arquivo = pd.concat([dados_arquivo, dados], ignore_index=True)
                
            #Passa todos os dados para o df
    ...
    def quebra_arquivo(self):
        
        import re
        
        # Pergunta ao usuário qual coluna ele deseja usar para separar os arquivos
        coluna = tk.simpledialog.askstring("Separar Arquivos", "Informe a coluna que deseja quebrar o arquivo: ")
        ...
        if coluna:
            ...
            # Pede ao usuário que selecione uma pasta para salvar os arquivos
            pasta_destino = filedialog.askdirectory(title="Selecione a pasta que deseja quebrar os arquivos")
            ...
            if pasta_destino:
                ...
                for valor, grupo in grupos:
                    ...
                    """
                    Essa parte [^\w\-_. ] é uma expressão regular (regex) que define um conjunto 
                    de caracteres que não são permitidos na criação de nomes de arquivos.

                    O conjunto de caracteres inclui:

                        ^: Indica a negação da classe de caracteres, ou seja, indica que os 
                        caracteres que se seguem não devem ser incluídos.
                        \w: Representa todos os caracteres alfanuméricos (letras e dígitos) 
                        e o caractere underscore (_).
                        \: É usado para escapar o próximo caractere, tornando-o literal. 
                        Neste caso, o - e o . são escapados, ou seja, são tratados como 
                        caracteres literais e não como metacaracteres.
                        -: Representa o caractere hífen.
                        _: Representa o caractere underscore.
                        .: Representa o caractere ponto.
                        : Representa o caractere espaço em branco.

                    Assim, essa parte da expressão regular [^\w\-_. ] significa "qualquer 
                    caractere que não seja alfanumérico, hífen, underscore, ponto ou espaço em branco".
                    """
