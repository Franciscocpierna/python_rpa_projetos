...
class ExcelEditor:
    ...
    def merge_left_join(self):
       ...
       # Realiza a junção (merge) dos dois DataFrames, arquivo1 e arquivo2, usando o método pd.merge do pandas:
        self.df = pd.merge(
            arquivo1,                # Primeiro DataFrame (à esquerda da junção)
            arquivo2,                # Segundo DataFrame (à direita da junção)
            on=["Id Vendedor"],      # Coluna usada como chave para juntar os dados dos dois DataFrames
            how="left",              # Tipo de junção: "left" (mantém todas as linhas do DataFrame da esquerda e adiciona dados do da direita quando houver correspondência)
            suffixes=(" Vendas", " Checagem") # Sufixos adicionados aos nomes das colunas que aparecem nos dois DataFrames para diferenciá-las
        )
