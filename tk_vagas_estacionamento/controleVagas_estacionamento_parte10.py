...
# Ajusta a largura das colunas da Treeview
def ajustar_largura_colunas(treeview, colunas, planilha):
                                                           
    # Esta função ajusta a largura das colunas em um widget Treeview
    # com base nos dados de uma planilha do Excel.
    
    # Cria um dicionário 'larguras' para armazenar as larguras iniciais de cada coluna.
    # A largura inicial é baseada no comprimento do nome da coluna.
    larguras = {col: len(col) for col in colunas}

    # Itera sobre cada linha da planilha do Excel, começando da segunda linha (min_row=2).
    # A opção 'values_only=True' garante que apenas os valores das células
    # sejam retornados, sem objetos de célula.
    for linha in planilha.iter_rows(min_row=2, values_only=True):
                                                           
        # Itera sobre cada coluna e seu respectivo valor na linha atual.
        # A função 'zip' combina a lista de colunas com os valores da linha atual.
        for col, valor in zip(colunas, linha):
            
            # Verifica se o valor da célula não é None, ou seja, a célula não está vazia.
            if valor is not None:
                
                # Atualiza a largura máxima registrada para a coluna.
                # Converte o valor para string e compara seu comprimento 
                # com o valor atual em 'larguras[col]'.
                # Se o comprimento da string do valor for maior, atualiza o 
                # valor em 'larguras[col]'.
                larguras[col] = max(larguras[col], len(str(valor)))

    # Após calcular as larguras máximas, ajusta a largura de cada coluna no Treeview.
    # Itera sobre cada coluna no Treeview.
    for col in colunas:
    
        # Define a largura da coluna no Treeview.
        # A largura é calculada multiplicando o maior comprimento
        # encontrado por 10 para obter uma largura visualmente adequada.
        treeview.column(col, width=larguras[col]*10)
...
def filtrar(*args):
    ...
    # Limpa a Treeview, removendo todos os itens existentes.
    # O método 'delete' com o argumento '*' remove todos os itens da Treeview.
    treeview_reservas.delete(*treeview_reservas.get_children())
    
    # Itera sobre cada linha da planilha 'planilha_reservas', começando
    # da segunda linha para ignorar o cabeçalho.
    # A opção 'values_only=True' garante que apenas os valores das 
    # células sejam considerados, sem os objetos de célula.
    for linha in planilha_reservas.iter_rows(min_row=2, values_only=True):
        
        # Inicialmente, assume-se que a linha satisfaz todos os critérios de filtro.
        # A variável 'coincide' é usada como um marcador para isso.
        coincide = True
        
        # Itera sobre cada coluna e a variável associada de filtro.
        # 'valor_filtro_var.items()' retorna pares de chave-valor, onde a 
        # chave é o nome da coluna e o valor é a variável de filtro correspondente.
        for col, var in valor_filtro_var.items():
            
            # Verifica se existe um filtro definido para a coluna atual ('var.get()' não é vazio) 
            # e se o valor na linha atual não corresponde ao valor do filtro.
            # 'linha[colunas.index(col)]' acessa o valor na linha atual na
            # posição correspondente à coluna atual.
            if var.get() and linha[colunas.index(col)] != var.get():
                
                # Se o valor na linha não corresponder ao filtro, a linha não 
                # coincide com os critérios de filtro.
                # A variável 'coincide' é marcada como False e a 
                coincide = False
                
                # Verificação desta linha é interrompida.
                break
                
        # Se a linha satisfaz todos os critérios de filtro (ou seja, 'coincide' é True),
        if coincide:
            
            # insere a linha na Treeview.
            # O método 'insert' adiciona um novo item na Treeview com os 
            # valores da linha que passou pelo filtro.
...
def reservas():
    ...
    fechar_button = tk.Button(janela_reservas, # Janela
                              text="Fechar", # 'text="Fechar"' define o texto do botão.
                              font=("Arial", 12), # Fonte e tamanho
                              command=janela_reservas.destroy) # 'command=janela_reservas.destroy' define a ação do botão para fechar a janela.
    
    

    # Posiciona o botão fechar usando o método grid
    fechar_button.grid(row=len(colunas)//3+2, # 'row=len(colunas)//3+2' posiciona o botão abaixo da Treeview.
                       column=0, # Posiona na coluna 0
                       columnspan=6, # 'pady=10' adiciona um espaçamento vertical acima e abaixo do botão.
                       pady=10) # Espaçamento em cima de 10
        
        
    # Ajusta a largura das colunas da árvore de exibição para caber no conteúdo
    ajustar_largura_colunas(treeview_reservas, colunas, planilha_reservas)
    
    
    # Definição da função abrir_detalhes, que será chamada quando um
    # evento ocorrer (geralmente um clique do mouse)
    def abrir_detalhes(event):
        
        # Esta função é chamada quando um evento específico ocorre, como um
        # clique do mouse em um item do treeview_reservas.

        # Obtém o item selecionado no treeview_reservas. O treeview_reservas
        # é um widget que mostra uma tabela ou lista na interface do usuário.
        # .selection()[0] obtém o primeiro item selecionado na tabela. Presume-se 
        # que apenas um item pode ser selecionado por vez.
        item_id = treeview_reservas.selection()[0]

        # Obtém os valores associados ao item selecionado na Treeview.
        # .item(item_id)["values"] acessa os dados da linha selecionada na tabela.
        valores = treeview_reservas.item(item_id)["values"]
                                                           
        # Cria uma nova janela para exibir os detalhes da reserva selecionada.
        # tk.Toplevel() cria uma nova janela. janela_reservas é a janela "pai" desta nova janela.
        janela_detalhes = tk.Toplevel(janela_reservas)

        # Define o título da janela de detalhes.
        janela_detalhes.title("Detalhes da Reserva")

        # Configura a cor de fundo da janela de detalhes para branco.
        janela_detalhes.configure(bg="white")

        # Cria uma lista para armazenar os widgets de entrada (como 
        # caixas de texto) que serão criados nesta função.
        # Esta lista 'entries' será usada para armazenar referências a
        # widgets que serão criados posteriormente no código.
        entries = []

        # Para cada valor na lista de valores, cria um label (texto descritivo) e
        # uma entrada de dados (caixa de texto)
        # Este loop passa por cada par de coluna e valor. 
        # 'enumerate' é usado para manter a contagem (i) de qual 
        # número do item estamos processando.
        # 'zip' combina as listas 'colunas' e 'valores' em pares para 
        # que possam ser processados juntos.
        for i, (col, val) in enumerate(zip(colunas, valores)):
            

            # Cria um rótulo (Label) para cada coluna e o posiciona na janela de detalhes.
            # 'ttk.Label' cria um rótulo com o texto sendo o nome da coluna.
            # '.grid' posiciona o rótulo na janela. 'row=i' significa que ele 
            # estará na linha i (a contagem do loop).
            # 'padx' e 'pady' adicionam um pouco de espaço ao redor do rótulo 
            # para evitar que ele toque outros elementos.
            ttk.Label(janela_detalhes, text=col, font="Arial 14").grid(row=i, column=0, padx=10, pady=10)
            
            # Se a coluna for "Status", cria um combobox (um menu suspenso) para a
            # entrada de dados
            # Esta condição verifica se a coluna atual é "Status". 
            # Se for, um Combobox é criado em vez de uma caixa de texto simples.
            if col == 'Status':

                # Cria uma variável StringVar para armazenar o valor selecionado no combobox.
                # 'StringVar()' é um tipo especial de variável do tkinter usado para armazenar strings.
                status_var = tk.StringVar()

                # Cria um combobox e associa a variável 'status_var' a ele.
                # 'ttk.Combobox' cria um menu suspenso. 'textvariable=status_var' 
                # vincula a variável ao valor do combobox.
                entry = ttk.Combobox(janela_detalhes, textvariable=status_var, font="Arial 14")

                # Define os valores possíveis para o combobox como 'Livre' e 'Reservado'.
                # Isso significa que o usuário só pode escolher entre 'Livre' e 'Reservado' no combobox.
                entry['values'] = ('Livre', 'Reservado')
                
                # Define o valor atual do combobox como sendo o valor 
                # associado à coluna 'Status' na linha selecionada.
                # 'set(val)' define o valor atual do combobox para o 
                # valor que está atualmente na linha selecionada da Treeview.
                entry.set(val)
                


           # Se a coluna for "Data de Saída", cria uma caixa de texto para a entrada de dados
            if col == 'Data de Saída':
                                                           
                # Se a coluna em questão for "Data de Saída", cria uma caixa 
                # de entrada (Entry) para que o usuário possa inserir ou editar essa informação.
                entry = ttk.Entry(janela_detalhes, font="Arial 14")

            # Se a coluna for "Hora de Saída", cria uma caixa de texto para a entrada de dados
            elif col == 'Hora de Saída':
                
                # Similar ao anterior, mas para a coluna "Hora de Saída".
                entry = ttk.Entry(janela_detalhes, font="Arial 14")

            # Se a coluna for "Total a Pagar", cria uma caixa de texto para a entrada de dados
            elif col == 'Total a Pagar':
                                                           
                # Para a coluna "Total a Pagar", cria-se também uma caixa de entrada.
                entry = ttk.Entry(janela_detalhes, font="Arial 14")
                
                
            # Se a coluna for "Total a Pagar", cria uma caixa de texto para a entrada de dados
            elif col == 'Status': # Esse aqui
                ...
                # 'state="readonly"' configura o combobox para ser somente leitura, o que significa 
                # que o usuário só pode selecionar os valores da lista, sem editar.
                entry = ttk.Combobox(janela_detalhes, values=valores_combo, font="Arial 14", state="readonly")
                                                           
                # Para a coluna "Total a Pagar", cria-se também uma caixa de entrada.
                # entry = ttk.Entry(janela_detalhes)

            # Para todas as outras colunas, cria uma caixa de texto para a entrada de dados
            else:
                
                # Para qualquer outra coluna que não seja as anteriores, cria-se uma caixa de entrada.
                entry = ttk.Entry(janela_detalhes, font="Arial 14")
                
                # Insere o valor atual (val) na posição inicial (0) da caixa de entrada.
                entry.insert(0, val)

            # Posiciona a caixa de texto na janela
            # 'entry.grid' posiciona a caixa de entrada na janela. 
            # 'row=i' determina a linha, e 'column=1' a coloca na segunda coluna.
            entry.grid(row=i, column=1, padx=10, pady=10)

            # Adiciona a caixa de texto à lista de entries
            # 'entries.append(entry)' adiciona a caixa de entrada criada à 
            # lista 'entries'. Isso é útil para referência futura ou manipulação.
            entries.append(entry)
            
            
        # Define uma função chamada 'salvar_alteracoes' que recebe dois parâmetros: 'item_id' e 'entries'.
        # 'item_id' é o identificador do item selecionado na Treeview, enquanto
        # 'entries' é uma lista de widgets Entry contendo os dados atualizados.
        def salvar_alteracoes(item_id, entries):
        

            # Carrega o workbook do Excel
            # Usa a biblioteca openpyxl para carregar a pasta de trabalho
            # do Excel. 'caminho_arquivo' é uma variável global definindo o caminho para o arquivo Excel.
            workbook = load_workbook(filename=caminho_arquivo)


            # Seleciona a planilha "Reservas"
            # Seleciona a planilha específica chamada "Reservas" dentro da
            # pasta de trabalho. 'planilha_nome' é uma variável global que armazena o nome da planilha.
            worksheet = workbook[planilha_nome]


            # Define os índices dos campos que devem ser usados para localizar a reserva
            # Esta linha cria uma lista chamada 'indices_campos_busca', que armazena os
            # índices das colunas (CPF, Data de Entrada, Número da Vaga e Bloco) 
            # usados para identificar a reserva específica na planilha.
            indices_campos_busca = [0, 2, 6, 7]


            # Encontra a linha correta com base no CPF, Data de Entrada, Vaga e Bloco
            # Itera sobre cada linha da planilha, começando da segunda 
            # linha (min_row=2) para pular o cabeçalho.
            for row in worksheet.iter_rows(min_row=2):

                # Usa uma compreensão de lista e a função 'all' para verificar 
                # se todos os valores das células nas colunas especificadas em 'indices_campos_busca' 
                # correspondem aos valores nas entradas (widgets Entry). 
                if all(row[i].value == entries[i].get() for i in indices_campos_busca):

                    # Atualiza os valores das células
                    # Itera simultaneamente sobre cada célula na 
                    # linha ('cell') e cada entrada no formulário ('entry').
                    for cell, entry in zip(row, entries):

                        # Atualiza o valor da célula com o valor atual da entrada correspondente.
                        cell.value = entry.get()

                    # Interrompe o loop assim que a linha correspondente for encontrada e atualizada.
                    break
                    

            # Salva o workbook
            # Salva as alterações feitas na planilha de trabalho do Excel.
            workbook.save(caminho_arquivo)
            

            # Atualiza a treeview
            # Chama uma função chamada 'atualizar_treeview'
            atualizar_treeview()
            
            
        # Adiciona um botão para salvar as alterações
        # Cria um botão utilizando o ttk.Button na janela de detalhes (janela_detalhes). 
        # O texto do botão é definido como "Salvar Alterações".
        # O parâmetro 'command' é configurado para chamar a função 'salvar_alteracoes' quando o botão for clicado. 
        # A função 'lambda' é usada para passar os argumentos 'item_id' e 'entries' para a função 'salvar_alteracoes'. 
        # 'item_id' identifica o item selecionado na Treeview e 'entries' é a
        # lista de widgets de entrada que contêm os valores atualizados.
        # O método .grid é usado para posicionar o botão na janela. 
        # O botão é colocado na linha correspondente ao número total de
        # colunas (len(colunas)), o que significa que será posicionado logo abaixo do último widget de entrada. 
        # O parâmetro 'column=0' indica que o botão deve ser colocado na primeira coluna da grade. 
        # 'padx=10' e 'pady=10' adicionam um espaço de 10 pixels ao redor do 
        # botão, tanto na horizontal (x) quanto na vertical (y), para melhorar a 
        # aparência e evitar que os elementos da interface fiquem muito juntos.
        ttk.Button(janela_detalhes, text="Salvar Alterações", 
                   command=lambda: salvar_alteracoes(item_id, entries)).grid(row=len(colunas), 
                   column=0, padx=10, pady=10)
            
            
        # Define a função 'atualizar_treeview', que não recebe parâmetros. 
        # Esta função é responsável por atualizar a Treeview com os 
        # dados mais recentes da planilha de reservas.
        def atualizar_treeview():
        
            # Limpa a treeview
            # Remove todos os itens existentes na Treeview. 
            # '*treeview_reservas.get_children()' obtém todos os itens (linhas) da 
            # Treeview e os deleta.
            treeview_reservas.delete(*treeview_reservas.get_children())

            # Carrega o DataFrame atualizado
            # Usa a biblioteca pandas para carregar os dados da 
            # planilha de reservas do arquivo Excel.
            # 'caminho_arquivo' especifica o caminho do arquivo Excel e
            # 'planilha_nome' é o nome da aba (sheet) dentro do arquivo.
            df_reservas = pd.read_excel(caminho_arquivo, sheet_name=planilha_nome)

            # Preenche a treeview com as linhas do DataFrame
            # Itera sobre cada linha do DataFrame. 'i' é o índice da
            # linha e 'row' é a própria linha (um objeto Series do pandas).
            for i, row in df_reservas.iterrows():

                # Insere cada linha do DataFrame na Treeview.
                # 'row.tolist()' converte a linha do DataFrame (Series) para uma lista, que é então inserida na Treeview.
                # "" e "end" especificam que a nova linha deve ser adicionada ao final da Treeview.
                treeview_reservas.insert("", "end", values=row.tolist())
                
                
        # Define a função 'excluir_reserva' que aceita 'item_id' e 'entries' como parâmetros.
        # 'item_id' é o identificador do item selecionado na Treeview.
        # 'entries' é a lista de widgets de entrada que contém os valores da reserva a ser excluída.
        def excluir_reserva(item_id, entries):
        
            # Carrega o workbook
            # Carrega o arquivo Excel especificado pelo caminho 'caminho_arquivo'.
            # Este arquivo contém a planilha com as reservas.
            workbook = load_workbook(filename=caminho_arquivo)


            # Seleciona a sheet "Reservas"
            # Seleciona a planilha chamada "Reservas" dentro do arquivo Excel.
            worksheet = workbook[planilha_nome]


            # Define os índices dos campos que devem ser usados para localizar a reserva
            # Define uma lista de índices que corresponde às colunas de interesse na planilha.
            # Estes índices são usados para identificar unicamente a reserva a ser excluída.
            indices_campos_busca = [0, 2, 6, 7]  # Correspondem a CPF, Data de Entrada, Vaga e Bloco


            # Encontra a linha correta com base no CPF, Data de Entrada, Vaga e Bloco
            # Itera sobre cada linha na planilha, começando da segunda linha (ignorando o cabeçalho).
            for row in worksheet.iter_rows(min_row=2):


                # Verifica se os valores nas colunas especificadas correspondem aos valores nos widgets de entrada.
                # Usa compreensão de lista e a função 'all' para verificar se todos os valores correspondem.
                if all(row[i].value == entries[i].get() for i in indices_campos_busca):


                    # Deleta a linha
                    # Se a linha correspondente for encontrada, a linha é deletada da planilha.
                    # 'row[0].row' obtém o número da linha atual na planilha.
                    worksheet.delete_rows(row[0].row)

                    # Interrompe o loop assim que a linha correta é encontrada e deletada.
                    break
            ...
            # Atualiza a treeview
            # Chama a função 'atualizar_treeview' para refletir as mudanças na Treeview.
            atualizar_treeview()
        
        


        # Adiciona um botão para excluir a reserva
        # Cria um novo botão na interface gráfica usando a biblioteca ttk, que é parte do Tkinter.
        # 'janela_detalhes' é a janela onde o botão será adicionado.
        # 'text="Excluir"' define o texto a ser exibido no botão.
        # 'command=lambda: excluir_reserva(item_id, entries)' define a ação a ser executada quando o botão é pressionado.
        # Neste caso, chama a função 'excluir_reserva' com os parâmetros 'item_id' e 'entries'.
        # 'item_id' é o identificador do item selecionado na Treeview, e 'entries' é uma lista dos
        # widgets de entrada que contêm os detalhes da reserva.
        # '.grid(row=len(colunas), column=1, padx=10, pady=10)' posiciona o botão na interface gráfica.
        # 'row=len(colunas)' posiciona o botão na linha abaixo da última coluna da Treeview.
        # 'column=1' coloca o botão na segunda coluna do layout.
        # 'padx=10, pady=10' adiciona um espaçamento de 10 pixels tanto na horizontal quanto na vertical ao redor do botão.
        ttk.Button(janela_detalhes, 
                   text="Excluir",
                   command=lambda: excluir_reserva(item_id, entries)).grid(row=len(colunas), column=1, padx=10, pady=10)
        ...
        # Esta função calcula o preço de uma reserva com base na data e hora de entrada e saída.
        def calcular_preco(data_entrada_str, hora_entrada_str, data_saida_str, hora_saida_str):
            ...
            # Usa 'strptime' para converter as strings de data e hora em
            # objetos datetime e time, respectivamente.
            ...
            # Combina as datas e horas em objetos datetime completos.
            # 'datetime.combine' é uma função que combina um objeto de
            # data com um objeto de hora.
            entrada = datetime.combine(data_entrada, hora_entrada)  # Cria um objeto datetime para o momento da entrada.
            saida = datetime.combine(data_saida, hora_saida)  # Cria um objeto datetime para o momento da saída.
            ...
            diferenca = saida - entrada  # Calcula a duração total da estadia.
            horas = diferenca.total_seconds() / 3600  # Converte a duração em segundos para horas.

            # Calcula o preço com base na política de cobrança.
            # O preço é 8 reais pela primeira hora e 1 real por cada hora adicional.
            preco = 8 + max(0, horas - 1)  # Calcula o preço total. Se menos de uma hora, cobra apenas 8 reais.

            # Retorna o preço calculado.
            return preco  # Retorna o valor final a ser cobrado pela estadia.
        
        # Busca as caixas de entrada (Entry widgets) para as datas e horas de entrada e saída, e o total a pagar.
        def atualizar_preco():
                                           
            # 'colunas.index' é usado para encontrar o índice correto da coluna na lista de colunas.
            entry_data_entrada = entries[colunas.index('Data de Entrada')]  # Encontra a caixa de entrada para a data de entrada.
            entry_hora_entrada = entries[colunas.index('Hora de Entrada')]  # Encontra a caixa de entrada para a hora de entrada.
            entry_data_saida = entries[colunas.index('Data de Saída')]  # Encontra a caixa de entrada para a data de saída.
            entry_hora_saida = entries[colunas.index('Hora de Saída')]  # Encontra a caixa de entrada para a hora de saída.
            entry_total = entries[colunas.index('Total a Pagar')]  # Encontra a caixa de entrada para o total a pagar.

            # Define a data e a hora de saída como as atuais.
            # 'datetime.now()' retorna a data e hora atuais.
            # 'strftime' formata a data e hora no formato desejado.
            data_saida_atual = datetime.now().strftime('%d/%m/%Y')  # Converte a data atual no formato dia/mês/ano.
            hora_saida_atual = datetime.now().strftime('%H:%M')  # Converte a hora atual no formato horas:minutos.

            # Obtém os valores das entradas
            data_entrada_str = entry_data_entrada.get()  # Obtém o valor da data de entrada a partir da caixa de entrada correspondente.
            hora_entrada_str = entry_hora_entrada.get()  # Obtém o valor da hora de entrada a partir da caixa de entrada correspondente.
            data_saida_str = entry_data_saida.get()      # Obtém o valor da data de saída a partir da caixa de entrada correspondente.
            hora_saida_str = entry_hora_saida.get()      # Obtém o valor da hora de saída a partir da caixa de entrada correspondente.

            # Calcula o preço
            # Chama a função 'calcular_preco' com os valores de data e hora de entrada e saída para calcular o custo da reserva.
            preco = calcular_preco(data_entrada_str, hora_entrada_str, data_saida_str, hora_saida_str)

            # Atualiza o campo 'Total a Pagar'
            entry_total.delete(0, tk.END)  # Limpa o campo de entrada do total a pagar.
                                           
            # Insere o preço calculado no campo de entrada, formatado como uma 
            # string com duas casas decimais.
            entry_total.insert(0, '{:,.2f}'.format(preco))

        # Adiciona um botão "Calcular Preço" na janela de detalhes
        # Este botão, quando clicado, chama a função 'atualizar_preco' para calcular e exibir o preço.
        ttk.Button(janela_detalhes, text="Calcular Preço",
                   command=atualizar_preco).grid(row=len(colunas) + 1, column=0, padx=10, pady=10)
        
        # Atualiza a interface gráfica para processar todos os eventos pendentes
        # Esta linha garante que todos os eventos pendentes (como atualizações
        # de layout) sejam processados antes de continuar.
        janela_detalhes.update_idletasks()

        # Obtém a largura atual da janela
        # Esta linha obtém a largura atual da janela de detalhes.
        largura = janela_detalhes.winfo_width()
        
        # Obtém a altura atual da janela
        # Esta linha obtém a altura atual da janela de detalhes.
        altura = janela_detalhes.winfo_height()        

        # Calcula a posição x para centralizar a janela na tela
        # Esta linha calcula a posição x (horizontal) para centralizar a janela na tela do usuário.
        posicao_x = (janela_detalhes.winfo_screenwidth() // 2) - (largura // 2)
        

        # Calcula a posição y para centralizar a janela na tela
        # Esta linha calcula a posição y (vertical) para centralizar a janela na tela do usuário.
        posicao_y = (janela_detalhes.winfo_screenheight() // 2) - (altura // 2)

        # Define a geometria da janela com a largura, altura, posição x e posição y
        # Esta linha define a geometria da janela, ajustando seu tamanho e posição na tela com base nos valores calculados.
        janela_detalhes.geometry('{}x{}+{}+{}'.format(largura, altura, posicao_x, posicao_y))
        
        
    # Associa um evento de duplo clique na Treeview com a função 'abrir_detalhes'
    # Quando um usuário faz um duplo clique em um item da Treeview, a 
    # função 'abrir_detalhes' é chamada.
    treeview_reservas.bind("<Double-1>", abrir_detalhes)
    
    # Atualiza a interface gráfica para processar todos os eventos pendentes
    # Esta linha garante que a janela processe qualquer evento 
    # pendente (como atualizações de layout) antes de continuar.
    janela_reservas.update_idletasks()

    # Obtém a largura atual da janela
    # Esta linha recupera a largura atual da janela de 
    # reservas para uso posterior no cálculo da posição.
    largura = janela_reservas.winfo_width()

    # Obtém a altura atual da janela
    # Da mesma forma, esta linha obtém a altura atual da janela de reservas.
    altura = janela_reservas.winfo_height()
    
    # Calcula a posição x para centralizar a janela na tela
    # Calcula a posição horizontal (x) para centralizar a janela 
    # na tela, levando em conta a largura da janela e da tela.
    posicao_x = (janela_reservas.winfo_screenwidth() // 2) - (largura // 2)

    # Calcula a posição y para centralizar a janela na tela
    # De maneira similar, esta linha calcula a posição vertical (y) 
    # para centralizar a janela, usando a altura da janela e da tela.
    posicao_y = (janela_reservas.winfo_screenheight() // 2) - (altura // 2)

    # Define a geometria da janela com a largura, altura, posição x e posição y
    # Finalmente, esta linha ajusta a geometria da janela de reservas usando os 
    # valores calculados para largura, altura e posição.
    janela_reservas.geometry('{}x{}+{}+{}'.format(largura, altura, posicao_x, posicao_y))
