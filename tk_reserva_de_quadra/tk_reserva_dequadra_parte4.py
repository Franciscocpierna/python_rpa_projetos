...
def detalhe_reservas():
    ...
    def filtrar():
        ...
        for linha in tree.get_children():
            ...
            tree.delete(linha) #Deleta todas as linhas existentes na treeview
            
        filtro = "Entre Datas"  # Define o filtro como "Entre Datas"
        data_inicio = entry_data_inicio.get()  # Obtém o valor inserido no campo entry_data_inicio
        data_fim = entry_data_fim.get()  # Obtém o valor inserido no campo entry_data_fim

        soma_preco = 0.0  # Variável para calcular a soma dos preços
        
        for row in dados[1:]:
            
            # Verifica se a data está dentro do intervalo especificado ou se não foi especificado nenhum intervalo
            
            """
            (not data_inicio or not data_fim): Esta parte verifica se não foi especificada 
            uma data de início ou uma data de fim. A expressão not data_inicio retorna 
            True se data_inicio for uma string vazia ou None, indicando que nenhum valor foi 
            inserido no campo entry_data_inicio. Da mesma forma, not data_fim retorna 
            True se data_fim for uma string vazia ou None, indicando que nenhum valor 
            foi inserido no campo entry_data_fim. Se pelo menos uma das duas condições for 
            verdadeira, significa que não foi especificado um intervalo de datas.

            (data_inicio <= row[0] <= data_fim): Esta parte verifica se a data da linha 
            atual (row[0]) está dentro do intervalo especificado. A expressão 
            data_inicio <= row[0] <= data_fim verifica se row[0] é maior ou igual a 
            data_inicio e menor ou igual a data_fim. Se data_inicio ou data_fim não foram 
            especificados (ou seja, são strings vazias ou None), essa parte da condição será 
            automaticamente verdadeira.
            """
            if (not data_inicio or not data_fim) or (data_inicio <= row[0] <= data_fim):
                
                """
                entries.values(): Aqui, entries é um dicionário que contém os campos Entry e 
                seus respectivos valores. entries.values() retorna uma lista com 
                todos os valores dos campos Entry.

                zip(entries.values(), row): Essa função zip combina os valores dos campos Entry 
                com os valores da linha atual, criando pares correspondentes.

                (entry.get().lower() in str(value).lower() for entry, value in 
                zip(entries.values(), row)): Aqui, utilizamos uma expressão geradora (generator 
                expression) para iterar sobre os pares de valores combinados. Para cada par 
                (entry, value), a expressão entry.get().lower() in str(value).lower() verifica 
                se o valor obtido do campo Entry (após ser convertido para minúsculas) está 
                presente no valor da linha atual (também convertido para minúsculas). Isso é 
                feito para cada par de valores.

                all(...): A função all verifica se todos os elementos na expressão geradora 
                são verdadeiros. Se todos os valores inseridos nos campos Entry corresponderem 
                aos valores da linha atual, o resultado será True.
                """
                if all(entry.get().lower() in str(value).lower() for entry, value in zip(entries.values(), row)): 
                    tree.insert("", tk.END, values=row)  # Insere a linha no treeview
                    
                    #soma_preco = soma_preco + ultima linha
                    soma_preco += row[-1]  # Adiciona o último valor da linha à soma dos preços
        
        label_valor_soma.config(text="{:.2f}".format(soma_preco))  # Atualiza o texto do label label_valor_soma com a soma dos preços formatada com duas casas decimais
    ...
    def preencher_campos(event):
        ...
        item = tree.focus() #Obtém o item atualmente selecionado no treeview tree.
        values = tree.item(item, "values") #Obtém os valores associados ao item selecionado no treeview tree.

        # Itera sobre os campos (chaves) e os campos de entrada (entry) do dicionário entries.
        for campo, entry in entries.items():
            
                entry.delete(0, tk.END) #Limpa o conteúdo do campo de entrada entry.
                
                # Insere o valor correspondente ao campo atual no campo de entrada entry, 
                #utilizando o índice do campo no dicionário entries para acessar o valor 
                #correspondente na lista values.
    ...
    def deletar_reserva(tree):
        ...
        cpf = entries["CPF"].get()  # Obtém o valor do campo "CPF" do dicionário `entries` e atribui à variável `cpf`.
        data = entries["Data"].get()  # Obtém o valor do campo "Data" do dicionário `entries` e atribui à variável `data`.
        hora_inicio = entries["Hora Início"].get()  # Obtém o valor do campo "Hora Início" do dicionário `entries` e atribui à variável `hora_inicio`.
        hora_fim = entries["Hora Fim"].get()  # Obtém o valor do campo "Hora Fim" do dicionário `entries` e atribui à variável `hora_fim`.
    
        if not cpf or not data or not hora_inicio or not hora_fim:
            
            label_status["text"] = "Por favor, preencha todos os campos."  # Exibe uma mensagem de erro se algum dos campos estiver vazio.
        ...
        reservas = sheet.iter_rows(values_only=True)  # Obtém um iterador para percorrer as linhas da planilha `sheet`, retornando apenas os valores sem formatação.
        next(reservas)  # Pula a primeira linha do iterador (cabeçalho), para começar a iteração a partir da segunda linha (dados).

        reserva_encontrada = False  # Variável para indicar se a reserva foi encontrada e excluída
        for i, reserva in enumerate(reservas, start=2):  # Itera sobre as reservas a partir da linha 2
            if str(reserva[3]) == cpf and str(reserva[0]) == data and str(reserva[1]) == hora_inicio and str(reserva[2]) == hora_fim:
                sheet.delete_rows(i)  # Exclui a linha da reserva encontrada
                reserva_encontrada = True  # Marca que a reserva foi encontrada
                break
                
        if reserva_encontrada:
            try:
                workbook.save(caminho)  # Salva as alterações no arquivo
                label_status["text"] = "Reserva excluída com sucesso."
                tree.delete(*tree.get_children())  # Remove todas as linhas da treeview

                # Insere os dados atualizados na treeview
                ...
                limpar_campos(tree, dados)  # Limpa os campos de entrada

            except Exception as e:
                label_status["text"] = "Erro ao salvar o arquivo: " + str(e)  # Exibe uma mensagem de erro se ocorrer algum problema ao salvar o arquivo
        else:
            label_status["text"] = "A reserva especificada não foi encontrada."  # Exibe uma mensagem se a reserva não for encontrada
    ...
    def limpar_campos(tree, dados):
        ...
        # Retorna os dados atualizados para serem utilizados na função deletar_reserva
        return dados
    
    botao_deletar = tk.Button(janela_detalhe_reservas,
                               text="Deletar Reserva", 
                               font=("Arial", 14),
                               command = lambda: deletar_reserva(tree),
                               width = 30)
    ...
    def exportar_para_excel(tree):
        ...
        #Obtém o caminho do arquivo
        caminho_destino = r"C:\Users\55119\Desktop\Reserva Quadra\DadosExportados.xlsx"
        planilha_destino = "Reservas Exportadas"

        try:

            workbook_destino = openpyxl.Workbook()
            sheet_destino = workbook_destino.active
            sheet_destino.title = planilha_destino
            
            #Obter cabeçalhos do treeview
            cabecalhos = [tree.heading(column)["text"] for column in tree["columns"]]

            # Escrever cabeçalhos na planilha de destino
            for col, cabecalho in enumerate(cabecalhos, start=1):
                
                sheet_destino.cell(row=1, column=col, value=cabecalho)
                
            #Obter os valores da Treeview
            valores = [tree.item(item)["values"] for item in tree.get_children()]
            
            #Escrever valores na planilha de destino
            for row, valores_row in enumerate(valores, start=2):
                
                for col, valor in enumerate(valores_row, start=1):
                    
                    sheet_destino.cell(row=row, column=col, value=valor)
                    
            workbook_destino.save(caminho_destino)
            
            messagebox.showinfo("Exportação concluída", "Os dados foram exportados para o arquivo DadosExportados.xlsx")
                    
            
        except Exception as e:

            messagebox.showerror("Erro", "Ocorreu um erro ao exportar os dados para o Excel: " + str(e))
            
        
        
    
    botao_exportar = tk.Button(janela_detalhe_reservas,
                               text="Exportar", 
                               font=("Arial", 14),
                               command = lambda: exportar_para_excel(tree),
                               width = 30)
    botao_exportar.pack(side="left", padx=10, pady=10)
...
btn_fazer_reserva = tk.Button(frame_botoes,
                             ...
                             width = 30)
btn_fazer_reserva.pack(pady = 10, padx = 30) # Coloca o frame na janela com um espaçamento vertical
